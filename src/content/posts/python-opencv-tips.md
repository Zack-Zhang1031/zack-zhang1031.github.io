...
学 OpenCV 的第一周，几乎所有人都会在"读图报错、imshow 崩溃、按键没反应"上卡半天。这篇笔记把日常用 OpenCV 最容易踩的小坑——读取失败判断、灰度化两种写法、视频流的 `0xFF` 按键检测、reshape 与 resize 的区别——集中整理，并附上 Python 字符串校验与 `any()` 的手写实现，方便随时查阅。
